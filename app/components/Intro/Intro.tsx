    ...
    <section ref={containerRef} className="w-full py-12 | xl:py-24">
      <div className="w-full px-4 | md:px-7">
        <div className="w-full flex justify-between items-start | flex-col-reverse | md:flex-row | gap-x-3 | md:gap-x-5 gap-y-3 | md:gap-y-5">
            ...
            {introData.ctas.map((cta, i) =>
              i === 0 ? (
                <CommontButton
                  ...
                  label={cta.text}
                  variant="white"
                  className="ring-1 ring-grey-900/5 w-full flex-row-reverse"
                />
              ) : (
                <Link
                  key={i}
          ...
          <div className="w-full grid | max-w-[24rem] | md:max-w-160 | xl:max-w-150 border | gap-y-3 | md:gap-y-7">
            <h2 className=" flex-wrap text-balance relative flex flex-col text-left justify-start text-grey-900 text-7xl font-sans-primary font-medium tracking-tight">
              <div className="flex flex-wrap relative text-left justify-start items-center">
                {introData.heading.split(" ").map((word, i, arr) => (
                  <React.Fragment key={i}>
                    ...
                    {i === arr.length - 1 && (
                      <div
                        className="inline shrink-0 flex bg-black/10 relative overflow-hidden mr-2 | pointer-fine:mr-0"
                        style={{
                          borderRadius: "15%",
                          width: "81px",
                          height: "45px",
              ...
              {introData.ctas.map((cta, i) =>
                i === 0 ? (
                  <CommontButton
                    ...
                    label={cta.text}
                    variant="white"
                    className="ring-1 ring-grey-900/5 flex-row-reverse"
                  />
                ) : (
                  <Link
                    key={i}
